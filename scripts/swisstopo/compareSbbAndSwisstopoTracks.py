...
def computeTrackPath(curvatureDf, initialHeadingDeg=0):
    ...
    ds = np.diff(positions)

    headings = np.zeros(len(positions))
    headings[0] = np.radians(initialHeadingDeg)

    headings[1:] = headings[0] + np.cumsum(curvatures[:-1] * ds)


    ### Compute track coordinates

    headingMid = 0.5 * (headings[:-1] + headings[1:])

    eastings = np.zeros(len(positions))
    northings = np.zeros(len(positions))

    eastings[1:] = np.cumsum(ds * np.cos(headingMid))
    northings[1:] = np.cumsum(ds * np.sin(headingMid))
...
if __name__ == '__main__':
    ...
    eastings, northings = computeTrackPath(SBB_track.curvaturesSigned, spacing=1, initialHeadingDeg=120)
